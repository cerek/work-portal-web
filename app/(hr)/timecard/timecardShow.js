...
export default function TimecardShow({ departmentList }) {
  ...
  const CheckEmployeeTimecard = async (empId) => {
    // Add schedule dataList [Done]
    // Add timecard dateList [Done]
    ...
    // Pull timecard data
    const timecardListRes = await fetch(
      '/api/timecard/' + empId + '/?clocking_record_datetime__gte=' +
        formattedPriorDate +
        '&clocking_record_datetime__lte=' +
        formattedFutureDate +
        '&page_size=400'
    )
    const timecardList = await timecardListRes.json()
    if ('error' in timecardList) {
      toast.current.show({
        severity: 'error',
        summary: 'Failed',
        detail: JSON.stringify(timecardList),
        life: 30000,
      })
      return
    }
    ...
    const timecardCalData = timecardList.results.map(function (element) {
      return {
        title: 'Punching',
    ...
    setCalData(scheduleCalData.concat(timecardCalData))
    toast.current.show({
      severity: 'success',
      summary: 'Successful',
    ...
    return (
      <React.Fragment>
        <Button
          ...
          rounded
          raised
          onClick={() => CheckEmployeeTimecard(rowData.id)}
        />
        <Link href={'/timecard/list/' + rowData.id}>
          <Button
            icon="pi pi-pencil"
            className="mr-2"
            severity="warning"
