...
export async function GET(req) {
  ...
  const backEndUrl = process.env.WORKPORTAL_BACKEND_URL + '/mytimeoffapplication/' + frontEndUrl.search
...
export async function POST(req) {
  ...
  const res = await fetch(process.env.WORKPORTAL_BACKEND_URL + '/mytimeoffapplication/', {
    method: "POST",
    headers: {
      'Authorization': 'Bearer ' + token.user.access,
      'Content-Type': 'application/json',
