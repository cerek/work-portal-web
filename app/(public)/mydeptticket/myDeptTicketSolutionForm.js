...
export default function MyDeptTicketSolutionForm({ ticketIns, currentEmpId }) {
  ...
  async function onSubmit(data) {
    ...
    const res = await fetch('/api/mydeptticket/' + id, {
      method: 'PATCH',
    ...
    if (!res.ok) {
      ...
      setEditSwitch(false)
      router.push('/mydeptticket/detail/' + id)
      router.refresh()
      toast.current.show({
        severity: 'success',
        summary: 'Successful',
            ...
            <InplaceContent>
              <form onSubmit={handleSubmit(onSubmit)}>
                  ...
                  render={({ field }) => (
                    <>
                      <Editor
                        ...
                        style={{ height: '320px' }}
                        readOnly={ticketIns.ticket_assigner === currentEmpId ? false : true}
                      />
                ...
                <div className="flex flex-wrap justify-content-between align-items-center gap-3 mt-3">
                  <Button type="submit" label="Save" disabled={ticketIns.ticket_assigner === currentEmpId ? false : true} />
                  <Button
                    type="button"
                    severity="danger"
                    label="Cancel"
